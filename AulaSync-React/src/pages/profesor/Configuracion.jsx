    ...
    return (
      <div className="flex items-center justify-center min-h-[80vh] bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <div
          ...
          style={{
            // SOLO EN MOVIL: tamaño más pequeño y padding reducido
            padding: window.innerWidth < 640 ? "2rem 1.2rem" : "4rem 4.5rem",
            minWidth: window.innerWidth < 640 ? 0 : 420,
            maxWidth: window.innerWidth < 640 ? 260 : 520,
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            <Loader2 className={`text-blue-500 animate-spin ${window.innerWidth < 640 ? "h-8 w-8" : "h-12 w-12"}`} />
            <span className={`font-bold text-blue-900 ${window.innerWidth < 640 ? "text-lg" : "text-2xl"}`}>AulaSync</span>
          </div>
          <div className={`flex flex-col gap-3 ${window.innerWidth < 640 ? "min-w-0" : "min-w-[300px]"}`}>
            {steps.map((s, idx) => (
              <div className="flex items-center gap-3" key={s.label}>
                {step > idx ? (
                  ...
                  <span className="w-4 h-4 flex items-center justify-center">
                    <span className={`rounded-full border-2 border-blue-600 border-t-transparent animate-spin ${window.innerWidth < 640 ? "w-3 h-3" : "w-4 h-4"}`}></span>
                  </span>
                ) : (
                  <span className="w-4 h-4 flex items-center justify-center">
                    <span className={`rounded-full border-2 border-gray-300 border-t-transparent ${window.innerWidth < 640 ? "w-3 h-3" : "w-4 h-4"}`}></span>
                  </span>
                )}
                <span className={`text-blue-800 ${step > idx ? "line-through text-blue-700" : ""} ${window.innerWidth < 640 ? "text-xs" : ""}`}>{s.label}</span>
              </div>
            ))}
          </div>
          <div className={`mt-8 text-blue-700 flex items-center gap-2 ${window.innerWidth < 640 ? "mt-4 text-xs" : "text-sm"}`}>
            Un momento, cargando configuración de perfil
            <span className={`inline-block text-blue-700 font-bold ${window.innerWidth < 640 ? "w-4" : "w-6"}`} style={{ letterSpacing: 1 }}>
              {".".repeat(dotCount + 1)}
            </span>
          </div>
          <style>{`
